...
| `test_full_e2e.py` | 完整 session 流、同 session 排队、跨 session 并行 | `test_session_lifecycle.py` + `test_job_scheduling.py` |
...
5. 删除旧文件：`test_agent_name_alignment.py`、`test_api_endpoints.py`、`test_full_e2e.py`、`test_tool_denylist_visibility.py`、`test_deepagent_integration.py`。
7. 运行全量 e2e 测试验证。
