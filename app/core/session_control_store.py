...
_THREAD_OWNER_BINDINGS_TABLE_DDL = """
...
class SessionControlStore(ThreadCatalogMixin):
    ...
    def _initialize(self) -> None:
        ...
        try:
            ...
            self._connection.execute(_THREAD_OWNER_BINDINGS_TABLE_DDL)
    ...
    def ensure_thread_owner_binding(
        ...
        with self._write_transaction() as connection:
            ...
            connection.execute(
                ...
                "VALUES (?, ?, 1, 'initial', '[]', '[]', '[]', '[]', '[]', "
                "'[]', 1, ?, ?)",
                (thread_id, final_relative_locator, now_text, now_text),
            )
            row = connection.execute(
                "SELECT * FROM thread_owner_bindings WHERE thread_id = ?",
                (thread_id,),
    ...
    def update_thread_owner_binding(
        ...
        *,
        ...
        if stable_prefix_hash is not None:
            if _SHA256_HEX_PATTERN.fullmatch(stable_prefix_hash) is None:
                raise ValueError(
