...
class BoxteamLiteLLMChatModel(LiteLLMHistoryProjectionMixin, ChatLiteLLM):
    ...
    async def astream(
        ...
        **kwargs: Any,
        ...
        """保留 LangChain model run ID，并传入 provider 的唯一 delta bridge。

        BaseChatModel 当前调用 ``_astream`` 时没有转发 ``run_manager``。本项目
        的 provider delta 需要和外层 ``on_chat_model_*`` 使用同一个 model-call
        身份，否则工具循环中下一次请求会把前一次的 block 当成迟到数据。这里
        只复制上游生命周期编排，唯一的行为差异是显式传递 ``run_manager``。
        """
        ...
        try:
            ...
            async for chunk in self._astream(
                ...
                **kwargs,
                ...
                if self.output_version == "v1":
                    ...
                    for block in cast("list[dict[str, Any]]", chunk.message.content):
            ...
            generations_with_error_metadata = _generate_response_from_error(error)
        ...
        if not generation:
            error = ValueError("No generation chunks were returned")
            await run_manager.on_llm_error(error, response=LLMResult(generations=[]))
            raise error
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        delta_sink = get_current_model_delta_sink()
        model_call_id = resolve_model_run_id(delta_sink, run_manager)
        attempts = self._stream_attempt_count()
        for attempt in range(1, attempts + 1):
            first_chunk_yielded = False
            part_state = _StreamPartState()
