...
def canonical_tool_pairings(
    ...
    逐字节一致），tool result 正文携带显式 tool_call_id。任何缺少显式
    链接的条目直接 fail closed，不做顺序/名称猜测。
    """
    ...
    for item in items:
        ...
        if item.semantic_kind == SemanticKind.TOOL_CALL.value:
            ...
            for call in calls:
                ...
                prior = call_ids.get(explicit)
                if prior is not None and prior != item.item_id:
                    raise ContextAssemblySealPreflightError(
                        "seal-dispatch-tool-pairing-conflict",
                        f"显式 call id {explicit!r} 被多个 call item 声明: "
                        f"{prior!r}, {item.item_id!r}",
                    )
                call_ids[explicit] = item.item_id
            ...
            prior = result_ids.get(explicit)
            if prior is not None and prior != item.item_id:
                raise ContextAssemblySealPreflightError(
                    "seal-dispatch-tool-pairing-conflict",
                    f"显式 tool_call_id {explicit!r} 被多个 result item 声明: "
                    f"{prior!r}, {item.item_id!r}",
                )
            result_ids[explicit] = item.item_id
    return tuple(
        (call_ref_id, result_ids[call_id])
        for call_id, call_ref_id in call_ids.items()
...
def _validate_tool_pairing(
    ...
    for call_ref_id, result_ref_id in tool_pairings:
        ...
        if call_ref_id in seen_calls or result_ref_id in seen_results:
            raise ContextAssemblySealPreflightError(
                "seal-dispatch-tool-pairing-conflict",
        ...
        seen_results.add(result_ref_id)
    unpaired = (set(calls) - seen_calls) | (set(results) - seen_results)
    if unpaired:
        raise ContextAssemblySealPreflightError(
            "seal-dispatch-tool-pairing-conflict",
