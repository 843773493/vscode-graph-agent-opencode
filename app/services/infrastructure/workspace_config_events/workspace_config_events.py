"""Workspace 状态库 config event outbox 与其 relay/consumer 投递账本的唯一实现。

本模块承载单一垂直链路的唯一实现：

- ``config_events`` 权威 outbox（幂等 append、按域分页读取、cursor 边界与裁剪、
  单 consumer relay 状态机）；
- ``config_event_relay_delivery`` 每个 consumer 独立的投递状态（claim /
  delivered / failed，含租约与重试时间）。

``WorkspaceConfigEventMixin`` 由
``app.services.infrastructure.workspace_state_store.WorkspaceStateStore`` 继承
装配；宿主负责 ``_WORKSPACE_MIGRATIONS`` 中本族两张表的 DDL 与迁移序号，本模块只
承载读写方法族，宿主提供 ``_database``。其它方法族（source journal、active
snapshot、pending candidate、apply claim/journal）在各自事务内通过
``self._insert_config_event(...)`` 复用本模块的唯一插入实现。

错误分类沿用 workspace_state_store 约定：``ValueError`` 输入形态非法、
``KeyError`` 目标事件缺失、``ConfigConflictError`` claim/幂等冲突、
``RuntimeError`` 事务后读取失败、``ConfigEventCursorGoneError`` 游标已被裁剪。
"""
...
# config_events 的完整行投影：单事件读取、relay 待投递与按域分页三处共用同一
# 列清单，新增列时只需改这里。ALIASED 版本供 LEFT JOIN config_event_relay_delivery
# 的 consumer 独立认领路径复用同一列序。
...
class WorkspaceConfigEventMixin:
