...
上述拆分的依赖方向固定为 `domain -> ports`、`storage/assembly/checkpoint -> domain + I/O`、`mapping -> domain/Saver DTO（无 I/O）`、`provider wrapper -> normalization/唯一 ToolSet bridge`、`orchestration -> ports/events`；storage 不得反向依赖 mapping/LangChain，mapping 不得依赖 storage/provider SDK，orchestration 不得保存 canonical/storage/provider 事实。资源平台新增目录由`add-context-injection-lifecycle`的design 5.3独占：观察来源、语义派生、Registry、VRN、业务loader和activation coordinator各自有单一owner；其`app/core/lifecycle.py`提供唯一进程内`LifetimeScope`，本change只消费该合同管理resident ThreadRuntime的可回收句柄，不复制dispose、watch共享或外部资源停止机制。本change只在既有`domain/itemized`与`infrastructure/rollout_context/{runtime,assembly,checkpoint,storage}`定义并持久化sealed resource refs，不复制目录树或建立第二source registry。每个新建 source subdirectory 必须在同一变更中创建包含“目录用途”“可修改内容”“不可修改内容”“规范”四部分的 `AGENTS.md`；已有目录沿用现有 `AGENTS.md`，不得重复创建。任何新增或重构源码文件超过 800 行、混合两个以上领域职责，或目标目录直接源码文件超过 20 个，均必须在 7.5-F 前拆分或提供包含文件清单、职责/owner 映射、import graph、行数统计和复核结论的架构审查证据；不得用长期 compatibility shim、dual writer、dual projector、双 schema 或双事实源规避该门槛。
...
├── extensions             # 必填 object；仅存命名空间化、版本化的非决策数据
...
├── projection_group?      # typed ProjectionGroup；只用于显式 message carrier 归组
├── projection_identity?   # typed ItemProjectionIdentity；part/message/model-call 身份
├── tool_result_evidence?  # typed ToolResultEvidence；纯文本结果身份与确认状态
├── source_manifest?       # typed ItemSourceManifest；source revision/保护/可见性
└── wire_role?             # 可选编码提示，不是来源或持久化分类
```

必填核心字段必须存在且非 null；`extensions` 即使无值也必须是空 object，`producer_ref` 必须是完整的单一 producer reference，`payload` 必须携带与 `payload_kind` 匹配的值。`content_hash` 的算法、输入和编码也属于核心合同：先构造恰好为 `{ "payload_kind": <payload_kind>, "payload": <payload> }` 的对象，再按 RFC 8785 JCS 生成无空白 UTF-8 字节，计算 SHA-256，并以 `sha256:jcs:v1:` 加 64 位小写 hexadecimal 编码保存。文本按原始 Unicode 字符串哈希，不 trim、不换行归一化；对象 key 由 JCS 排序，数组保留 payload 的语义顺序；二进制必须先由 payload schema 明确编码为 base64url 等 typed value。item identity、sequence、semantic kind、status、producer、时间、extensions、typed 关联/投影字段、wire role 和 detail path 均不进入 `content_hash`；正文哈希不替代不可变 JSONL 行与 SQLite committed-boundary 的完整性校验。`turn_id`、`turn_scope`、`message_group_id`、`projection_group`、`projection_identity`、`tool_result_evidence`、`source_manifest` 和 `wire_role` 是按语义可空/可省略的关联或投影字段。语义约束固定为：`turn_scope=turn_root` 必须同时有非空 `turn_id`，且该 item 必须是该 Turn 唯一的 `user_input` root；普通 Turn item 必须 `turn_scope=turn_member` 且有非空 `turn_id`；反向地，任何非空 `turn_id` 都必须配合 `turn_root` 或 `turn_member`，不得出现 `turn_id != NULL` 且 `turn_scope=NULL`；`turn_scope=ambient` 或 `pending_next_turn` 必须 `turn_id=NULL`，不能进入普通 Turn member/root 集合；`semantic_kind=runtime_notice` 的 pending notice 必须使用 `turn_scope=pending_next_turn`，而 request-only runtime notice 不产生 CanonicalItemRecord。若三个 Turn 关联字段均为 null，该 item 不属于任何 Turn，reader 不得从物理邻接、wire role 或 message group 推断其属于某个 Turn。`message_group_id` 为空不影响 item 的 canonical 顺序；非空时也不能改变上述 root/member/ambient 约束。

核心字段闭合不是把所有 annotation 都做成顶层列。`ProjectionGroup(ordinal: int, size: int, content_form: "str"|"list")`只在确实拆成多个 canonical item、需要重建同一 message carrier 时存在：`size >= 2`、`0 <= ordinal < size`、同一 `message_group_id` 的成员恰好覆盖 `0..size-1`，anchor 是最后一个 tool-call item；单 item 不伪造 group。`ItemProjectionIdentity` 持有被 history/checkpoint/fork 用于同一逻辑 item 识别的 `projection_message_id?`、`model_call_id?`、`block_id?`、`block_index?`、`content_part_refs[]` 和已知 `reasoning_carrier?`；它们的可空性、ref 类型、顺序与 carrier schema 在 domain 校验，不能因正文相等、时间邻接或缺字段而回退猜测。`ToolResultEvidence(tool_call_id, result_id, execution_confirmed)` 是纯文本 tool result 的必需关联事实，结构化结果仍以 typed payload 中的身份/`tool_outcome` 为准，两种表示在 seal 前互斥且一致。`ItemSourceManifest` 仅存 canonical source 的语义 revision、visibility/protection；真正的 producer、关系边和已提交 source catalog 各守其 owner，不能复制出第二权威。`supersedes_message_id`/fork lineage 改为 typed item/provenance relation，attachment 归 payload/attachment reference，不再通过自由字段决定投影或复制。

`extensions` 的 key 必须是稳定 namespaced schema identifier，value 必须是 `{extension_version, value, protection, encoding?}`；未知但结构合法的扩展按原字节/版本保留，不得参与 Turn 归属、selection、source 顺序、replacement、tool 成功、final item、history 去重、hash/dispatch 安全或恢复判断。`payload_kind=extension` 是 canonical 正文的另一合同，不等于此处 envelope 的非决策 `extensions`。确需让新增生产代码影响核心行为时，先扩展有 owner、类型、校验、版本与测试的核心 capability/intent，再由相应producer提交该 typed intent；不得在 `extensions` 中放一个隐式开关。domain 类型字段与中文注释、序列化 schema、SQLite 约束和 API/Proto 投影必须同名或有明确映射表，缺失/非法核心字段在写入与恢复两侧 fail closed。凡现有 `metadata.get(...)` 会决定上述行为的 key 都必须纳入迁移清单并移到明确的 typed owner；诊断/展示 annotation 可留在 `extensions`，但不得反向成为决策输入。

字段发现入口不是另建运行时万能 field registry，而是既有 domain 类型与目录指引：`app/domain/itemized/records.py` 定义 item 与上述投影/工具/source值对象，`request_plan.py` 定义 contribution 的选择/替换/source binding，`enums.py` 放闭合判别值，`schema.py`/`serialization.py` 分别验证与编解码；`app/services/infrastructure/rollout_context/assembly/registry.py` 负责 `RegisteredContribution` 的持久 ordinal，`storage/schema.py` 只镜像必要索引/约束而不重新定义语义。`app/domain/itemized/AGENTS.md` 应维护这张 owner 导航表和“新增决策字段先改类型/校验/序列化/存储/测试”的流程；源码每个核心字段要有中文业务语义、适用状态、可空条件和禁止的 fallback 注释。schema/serialization golden fixture 与静态审计验证这些映射，不能靠一份手写字段表代替可运行约束。这里的 source/projection 值对象按职责拆分，禁止把所有扩展字段拉平成巨大 `CanonicalItemRecord` 或复制到多个第二事实源。
...
| `tool_result` | `text`, `structured_content`, `tool_result`, `opaque`, `extension` | `completed`, `partial`, `incomplete`, `cancelled`, `failed`, `unknown` | text 的 call/result identity 与确认状态必须在 typed `tool_result_evidence`，其它 payload 按自身 typed schema 保存 identity/确认状态；`status=completed` 时 `tool_outcome` 可为 `success|failure|cancelled|unknown`，但未确认结果必须用能承载 `tool_outcome=unknown` 的 typed payload，不能选 text；其它 status 只能省略 marker 或使用 `unknown`；只有 `status=completed` 且 `tool_outcome=success` 才可 replay |
...
本 change 的实验 v2 顶层 envelope 以本节的新合同为准：必填非空字段为 `format_version=2`、`record_type=item`、`item_sequence`、`item_id`、`semantic_kind`、`payload_kind`、`status`、`producer_ref`、`payload`、`content_hash`、`created_at`、`extensions`；按语义可空字段为 `turn_id`、`turn_scope`、`message_group_id`、`projection_group`、`projection_identity`、`tool_result_evidence`、`source_manifest` 和 `wire_role`。`metadata` 只属于 v1 legacy envelope 或非 canonical 的独立协议对象，新的 v2 item-line 禁止携带它。该字段形态直接取代尚未发布的旧实验 v2 合同：不维护旧 v2 reader、双写、别名或自动迁移，打开旧形态必须明确报 schema 不兼容并保持原数据不动；测试/开发数据由显式重建处理。SQLite manifest/database metadata 的 `rollout_format_version=2` 必须与每行 `format_version=2` 和 `record_type=item` 一致。v1 legacy envelope 仍固定为 `format_version=1`、`record_type=message`、`message_sequence`、`message_id`、`turn_id`、`role`、`message` 和 `metadata`；原有一次性 v1 importer 独立保留，v1/v2 不得在同一个 rollout 中混写。`rollout_format_version` 是存储 dispatch 字段，`format_version` 是每行协议字段，两者职责不同但必须一致。
...
Node调试方案属于thread-owned、非canonical的可移植资源，fork复制范围与历史item范围分开定义：同Workspace `context_fork`只带source选中thread在capture时活动方案的当前正文，`history_prefix_fork`不带方案（历史anchor不具备对应调试方案快照），`full_rollout_copy`带全部当前已保存方案正文；内部`materialize_thread_copy`不自动带方案。target只保存target-local方案ID/lineage，不复制active指针、历史方案revision、进程、端口或动作审计，不能据`full`字样推断后者可恢复。debug owner在source read guard下固定方案manifest/revision并对逐文件bytes/hash做前后校验，将选中artifact列入同一个SourceCopySnapshot；target debug owner在未发布staging中冻结有效Workspace debug配置revision/hash，以该快照逐一校验入口、工作目录、每个断点路径、profile/adapter/runtime，并在发布前复核配置revision仍相同。任何必需方案缺失/漂移/不兼容或目标配置漂移使整个fork失败，不能只丢方案仍发布target；fork journal记录source artifact、target配置hash及方案映射并承担恢复，不另建copy coordinator。公开fork不跨Workspace，未来跨Workspace可移植方案转移须另设授权export/import，Gateway federation的send/read/wait grant不能被复用成复制许可。
...
例如 `AGENTS=A(base) + A→B(delta)` 后 rewind 到不再包含最新已注入 revision 的历史边界，rewind实际重建新prefix epoch；若目标 checkpoint 仍保留 tracked registration，CSM从配置边界冻结的ResourceActivationSnapshot取得当前有效published revision B并持久化恢复事实，建立新overlay epoch，而不是把旧A→B delta越过rewind cutoff重新注入。若该source由owner声明`root_eligible`，新完整状态可合并到新root；`tail_only`恢复为独立user item。snapshot/untracked source不自动恢复。若required ResourceSnapshot不可用则rewind view仍可提交，但后续dispatch显式失败，不能请求期读盘或用当前内容修补旧assembly。
...
├── selection_role           # direct | backing_only；是否允许独立进入 selection
├── replacement_policy       # immutable | replaceable；同 source slot 的更新准入
├── root_placement?          # source-backed时必填：root_eligible | tail_only；由source owner声明
├── source_binding?          # typed SourceBinding；source/overlay ref、role、epoch
├── body/detail_ref
...
├── extensions              # namespaced/versioned 非决策注释；默认空 object
└── ordinal_binding?
    ├── assembly_id
    └── contribution_ordinal

RegisteredContribution      # registry 返回的 typed binding，不是 contribution 本体
├── contribution             # 不带 source_ordinal 的 immutable ContextContribution
└── source_ordinal           # registry 在 owner thread 内持久分配；非负且不重用
...
`ContextContribution.selection_role=direct|backing_only` 替代自由 `selection_only` flag：`backing_only` 只作为其它显式 selection ref 的 source backing，不得自行占一个 `plan_ordinal`；`direct` 仍需通过 selection authority 明确选入。`replacement_policy=immutable|replaceable` 替代自由 `replaceable_source` flag：只有同 owner、同 source slot、显式 `replaceable` 且 revision/CAS 匹配才可替换 unsealed registry 当前值，已提交 contribution/selection/JSONL 永远不原地更新。`SourceBinding` 使用 typed `source_ref?`、`overlay_ref?`、`overlay_id?`、`overlay_role=none|base|delta`、`source_overlay_epoch?` 表达现有 alias/overlay 映射；overlay 字段必须与 `contribution_kind` 及 `ContextSelectionEntry` 的 role/epoch 一致，非 overlay 禁止填 overlay ref/role/epoch。它是 producer 提供的来源身份，是否选入及最终顺序仍只由 sealed selection 决定。

`source_ordinal` 是 owner-thread registry 的持久分配结果，不是 contribution 的 payload、metadata、extension 或 assembly ordinal。registry `register/update` 返回 `RegisteredContribution(contribution, source_ordinal)`；ledger、composer 和 plan builder 只接收该 typed binding，不各自补造 fallback ordinal，也不得在 seal 时把该字段复制进 contribution 的 `extensions`。同一 source slot 的合法 replace 保留 ordinal，不同 source 新分配；SQLite 对 `(session_id, thread_id, source_ordinal)` 建唯一约束。`plan_hash` 最终只以 sealed `selection.plan_ordinal` 与被选 source manifest 表达语义顺序，不能让 registry 迭代顺序或扩展 key 改写它。
...
ContextRequestPlan 保留 prompt contribution 的独立 ref、顺序、来源及owner声明的typed `root_placement`。同epoch中，真实用户消息之后的runtime notice、compaction summary、Skill/AGENTS/team/MCP指引full或delta及其它source contribution都按plan ordinal投影为独立`wire_role=user` item，不得提升、前插或合并。只有首次assembly、实际compaction/rewind、Provider ToolSet hard rebase及fork目标首次assembly形成新epoch时，root compiler可将`root_eligible`受信来源当前有效完整状态编译成唯一最顶层system item；`tail_only`包括默认外部MCP指引，仍独立user item。新plan需记录被物化的base/delta lineage并避免同一内容重复选择，旧canonical/detail/sealed assembly字节不变。当前只为Anthropic部分官方模型保留关闭的mid-context system capability TODO。plan 必须分别携带 `history_view_revision` 与 `source_overlay_epoch`；缓存保持型 source 必须同时保留稳定 base 和有序 delta ref。应用只发生在 wire projector，不能把 delta 反向写入旧 base。wire role 不能抹掉 source identity、request-only/canonical 生命周期或 omission/loss 信息。`ToolSetSnapshot` 只编码Provider可见直接工具与固定`invoke_extension_tool`信封到tools/tool-config，不含内部extension target；工具定义不变成历史message。plan sealed 后不能用当前 middleware 配置静默重写旧 contribution；只能创建新的 assembly 并记录 source mismatch。

#### 5.1.2 Provider ToolSetRef 与扩展目录 binding 的封存边界

`ToolSetRef`及其`tool_set_manifest[]`只包含Provider `tools`实际可见的直接工具和固定`invoke_extension_tool(tool_name, arguments)`信封；内层MCP/内置/自定义target的名称、schema、策略和连接generation不得混进ToolSetRef或信封description。MCP工具目录变化是独立`ExtensionCatalogBindingRef`与其派生CSM指引revision变化，不是Provider tool schema变化，不单独创建`toolset_changed` epoch。只有直接工具或信封本身有效改变时才按生命周期change做ToolSet hard rebase。`ExtensionCatalogBindingRef`由统一Saver/ContextStore owner随model-call assembly seal持久化，至少绑定owner thread、Turn/ModelCall资源激活snapshot、catalog semantic revision/hash、指引revision/source ref、effective boundary、稳定target identity/schema hash、server generation验证ref与policy快照；连接实例ID不得成为业务identity。catalog正文/映射在受保护snapshot/detail中保存，不泄露endpoint或credential。sealed调用以这个ref解析目标；tool invocation/result另保存内层target identity、schema revision和调用generation，原Provider tool name仍是信封。

catalog binding与指引source在同一`turn|model_call`边界冻结：默认Turn不漂移；选用`model_call`时下一安全preparation先收敛旧call/result，再共同选中最新已发布目录与增量指引。catalog变化可改变新assembly的binding/provenance与plan selection/hash，但在Provider `tools`和已经提交的wire context前缀不变时，不得将其误判为ToolSet hard rebase；指引只作为独立user-role source item尾部追加。request-only binding不计入Turn的`item_count`或`elapsed_ms`。retry/exact replay只使用已封存目录与指引，不按当前MCP server或URI重新解析；fork target复制或重新封存target-local binding和历史lineage，不复制可执行旧connection handle。旧assembly字节及ToolSetRef不变。
...
- 同一prefix epoch最顶层的root contributions编译为唯一LangChain `SystemMessage`或Provider等价root instructions；合法新epoch可吸收`root_eligible`受信source当前完整状态，普通同epoch新增和所有`tail_only` source必须按plan顺序编译为相互独立的`HumanMessage`/Provider user item，不得前插、提升或合并。canonical `runtime_notice`/`compaction_summary`保留自身语义与顺序，不借root编译吞并。user input仍通过`root_input_item_id`识别Turn起点，user text/attachment编译为自己的`HumanMessage`或content list；
...
通用`SessionOperationLease`最少持久化`lease_id`、`operation_kind=thread_creation|board_migration|collaboration_fanout|runtime_owner|execution|context_control|debug_control|communication_source|communication_target|federated_call|remote_observation|attachment|fork_retention|session_catalog_mutation`、稳定`operation_identity`及preimage hash、捕获的`lifecycle_generation`、`holder_generation/fencing_token`、`state=active|settling|completed|cancelled|failed`、revision和可选`recovery_ref`，并对同generation/operation identity唯一、对非终态状态建立索引；独立Web/API调试mutation用`debug_control`准入，Agent内部可由execution lease覆盖，短期Session准入不替代跨Turn`node_debug_process`资源lease。专用record承担lease时必须提供同等字段/约束，并以显式`lease_state`或版本化全映射把自己的preparing/routing/published/aborted等状态归入上述非终态/terminal集合，禁止靠名称猜测。lease没有墙钟自动到期语义。进程退出后，恢复owner只有在验证旧holder generation失效并CAS取得新fencing token后才能继续原operation或把它推进为`settling`；不能把“看不到内存对象”当作terminal。fence转为deleting后禁止建立新lease，旧generation lease只能按其冻结preimage完成或取消原operation，不能派生新root/wakeup/child；删除owner可CAS请求settling，但必须等原writer确认或由幂等恢复逻辑核对外部commit后写terminal。跨库主体先durable提交，再把lease terminal；两者之间崩溃按`operation_identity + recovery_ref`核对同一结果，不重复副作用。terminal后任何携带旧token的callback都必须失败。同一Session的catalog locator/lifecycle/归档等目标条目mutation也必须以`session_catalog_mutation`竞争gate；否则属于不受支持的旁路写入并fail closed。
...
- 调试owner已核实该thread没有未结清的`launch_pending`进程claim、`starting|running|paused|stopping`进程，也没有未完成恢复/停止的`reconcile_required`阻断；活动调试不累计闲置时长，终态核实并结清lease后重新起算30分钟；
...
目标`app/services/orchestration/thread_residency.py`中的residency owner先依据上述条件和generation fence作业务准入决定，再关闭该generation的`LifetimeScope`；scope只负责取消/排空其持有的task并释放compiled invocation、模型/工具 client、in-memory ContextStore/CSM cache、stream execution fanout等可重建句柄，不计算30分钟、不更改status/ledger或停止进程级共享watch和terminal/browser/MCP等跨Turn外部资源。关闭完成后才能把residency投影推进cold；释放失败必须显式报告，不得伪报cold或让旧generation继续提交callback。thread catalog、GraphBinding、canonical JSONL、SQLite checkpoint/view、CSM registration/revision、ToolSet applied binding、等待所需的 Job 状态与历史都保持不变。普通 list/get/history/detail 和右侧侧边栏浏览走 cold metadata/history read path，不得 materialize Agent runtime。新的真实消息、内部 wakeup 或需要模型执行的恢复操作在 execution admission 时获取唯一 runtime generation/lease，从持久状态重建同一 `ThreadRuntimeBinding`；tracked source继续由共享resource monitor/reconciler异步更新，runtime只在配置的activation boundary消费Registry内存snapshot，不在before-model读源。idle unload 不创建 context item、不推进 prefix epoch、不使 Skill/上下文自动到期，也不等价于删除、停止或归档 thread。

调试进程是跨Turn业务资源，其真实状态及持久lease由debug owner提供；residency owner只消费已核实的blocker和终态时间，不从进程内scope关闭、缓存缺项或wall-clock猜停止/自动到期。无法核实或stop失败时继续保持resident及脱敏`debug_reconcile_required`阻断；`LifetimeScope`不负责杀Node进程。
...
2b. 此共享maintenance gate/journal必须静态接线调试domain的确定性迁移步骤，不建立plugin registry或第二迁移协调器：以已冻结的旧Session→main thread映射，将旧Session节点`debug/node/`的归属manifest与可移植方案逐文件登记、校验bytes/hash和ID/revision/lineage，再定点落入已解析main thread节点`debug/node/`。调试owner负责文件语义、进程收敛、方案归属和恢复；itemized owner负责同一staging/切换点、已登记artifact完整性与失败保留原件。没有成功的调试步骤证明时不得把含旧调试数据的Session发布为已完整迁移；普通runtime不提供旧路径alias或扫描目录补缺。
