...
`accepted_ingress_id` 与 `acceptance_idempotency_key` 必须分别建立 `(session_id, accepted_ingress_id)` 和 `(session_id, acceptance_idempotency_key)` 唯一约束，并各自一对一指向 accepted Turn。相同 key、ingress、payload hash 和 source branch 的重试返回原 Turn/root/initial execution；key 与 ingress/payload/branch 任一冲突，或 ingress 被不同 key 重用，必须返回明确 acceptance idempotency conflict，不创建或修改 Turn。跨 session fork/migration 生成的 copied/synthetic 值也必须占用 target session 的本地唯一空间并标记其 identity origin，不能使用 source 裸值绕过约束。

系统 SHALL 使用单个 `index.sqlite` 保存冻结的 v2 `rollout_format_version`/schema version、canonical item catalog、权威 Turn/Execution/ModelCall lineage、工具投影、reasoning 投影、checkpoint、branch、context view、assembly outcome、fork 来源、retention、`storage_commits` 和提交状态；一次性 migration staging/report 可以另存 v1 source coordinate/offset，但该坐标不提供正常 v1 读取 API。`TurnRecord` 至少保存 `turn_id`、session-global `turn_ordinal`、`accepted_ingress_id`、`acceptance_idempotency_key`、`root_input_item_id`、`initial_execution_id`、`last_execution_id?`、`final_item_id?`、status 和 origin `source_branch_id`。SQLite 不得被定义为可以仅从 JSONL 重建完整控制状态的缓存。

#### Scenario: SQLite schema 迁移

- **WHEN** 软件升级需要增加 item catalog、Turn root 或 assembly 表结构
- **THEN** 系统通过 `schema_migrations` 在事务中执行有序 migration，并更新 `database_meta.schema_version` 与 rollout format metadata
...
系统 SHALL 使用不可变 `context_views` 和 item/message range/reference 表达有效 canonical history context。view 可以引用其它 view 的逻辑范围，或引用 JSONL item/message 范围，但不得复制完整 messages/items 数组。`context_view_turns` 必须保存 view-local 唯一的 `logical_turn_ordinal`、`turn_id`、`root_input_item_id` 和 fork lineage；它不改变 `TurnRecord.turn_ordinal`、`source_branch_id` 或 root identity。request-only reference 和 source overlay reference 不属于普通 canonical history view membership；assembly 在解析 view 后必须独立执行 context reconciliation，必要时重新注入仍有效的 ambient delta。pending runtime notice 只能作为 pending/ambient reference 参与后续 assembly，不得成为普通 Turn root 或分页成员。`context_view_jumps` SHALL 支持长 view 链的快速跳转和 cycle 检查。
...
存储 SHALL 将缓存保持型 source overlay 的 base、delta、supersedes/materializes relation、`source_overlay_epoch` 和物化结果作为可校验的 assembly/overlay 引用保存，并与 `history_view_revision` 分开。rewind、replay、fork 或 compaction 造成 history view 变化时，只要 overlay 仍兼容，必须保留相同的 source overlay epoch 和 base/delta identity；被新 history view 隐藏的 delta 仍可由 overlay lineage 找回并重新注入。已物化的新完整 source revision 只能在新的 context view/assembly 中成为 active；旧 base/delta 的 JSONL item 或 request-only detail reference 保持不可变并可按 retention policy 查询，不得物理覆盖。source diff 如果需要跨 checkpoint 继续生效，必须有对应的 canonical ambient `runtime_notice` item catalog/关系；request-only base/delta 只能由 sealed assembly/detail reference 恢复。
...
- **WHEN** compaction 或其它 source cache boundary invalidation 判定必须将 source B 物化为新的完整 revision
- **THEN** 新 active view/assembly 只引用 B 的完整 revision 和新的 `source_overlay_epoch`，旧 A/A→B 引用仍可审计但不再被 projector 发送；若 reconciliation 判定 overlay 仍兼容，则 compaction 只推进 history view revision，继续引用 A/A→B，不强制物化
...
#### Scenario: history view 变化后重新注入 delta

- **WHEN** rewind 使保存 A→B delta 的历史 item 不再属于 active history view，但 source overlay epoch 和 detail 仍有效
- **THEN** 新 assembly 使用新的 history view revision，同时从已提交 overlay lineage 读取并重新注入 A→B；不得因为 delta 不在 history range 中而物化 A 或丢失 B
