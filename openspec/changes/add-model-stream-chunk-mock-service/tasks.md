...
- [x] 2.5 实现 `anthropic_messages_sse` codec，校验 `message_stop` terminal 和原生 JSON event。
...
- [x] 3.5 增加 Anthropic Messages reasoning/tool 手写 cassette，并验证 thinking、tool_use、text 和 message_stop。
...
- [x] 5.1 让 replay transport 根据 cassette protocol 获取 codec，并支持 Anthropic 手写 asset 的 wire replay。
...
- [x] 6.6 增加 Responses 到业务输出的集成断言，并为 Anthropic 增加 codec/wire round-trip 校验；不访问真实 Anthropic provider。
...
- [x] 7.5 增加 unknown protocol、Anthropic 非法 event/terminal 和 Responses 非法 terminal 的明确错误断言。
...
## 11. Responses 交错并发工具调用

- [x] 11.1 增加双 `read_file` 的 Responses 手写 cassette、scenario、expectation 和显式 JSONC 配置，覆盖两个 function call item 及交错 arguments delta。
- [x] 11.2 修正 Responses provider 到 LangChain chunk 的工具调用身份传递，为每个 `item_id/call_id` 保留独立参数累积状态。
- [x] 11.3 增加 provider/parser 单元测试，验证交错 delta 不会改变两个工具的名称、参数和 call_id。
- [x] 11.4 增加集成测试和 E2E，验证两个工具并发执行、结果按 call_id 回填、不同完成顺序不串线且最终文本正确。
- [x] 11.5 运行静态分析、相关 pytest 和 OpenSpec strict 校验，审查并发场景的上游与业务产物。
