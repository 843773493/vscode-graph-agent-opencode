...
投影 SHALL 保留用户消息、附件身份和相对路径元信息、可见 assistant 文本、标准工具调用和工具结果；目标 provider 不支持的用户 rich block 或 reasoning 必须仅在请求投影中过滤，并不得改写 checkpoint、rollout JSONL 或源消息。LiteLLM SHALL 优先用于 provider 间可统一的 content 转换，无法统一的 provider block SHALL 由目标 adapter 明确处理并返回可诊断的投影结果。
