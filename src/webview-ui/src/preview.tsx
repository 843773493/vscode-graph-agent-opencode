...
type PreviewUiState = Partial<AppState>;
...
// 这里模拟的是“后端原始消息格式”，不是最终前端展示结果。
const previewBackendPayload: PreviewBackendPayload = {
...
function buildPreviewStateFromBackend(payload: PreviewBackendPayload): PreviewUiState {
  return {
...
function injectPreviewState() {
  ...
  boot.textContent = JSON.stringify(previewState).split('<').join('\u003c');
