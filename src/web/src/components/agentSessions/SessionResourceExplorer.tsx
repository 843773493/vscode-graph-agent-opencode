...
interface SessionFolderContextMenu {
  workspaceId: string;
  folderId: string;
...
interface WorkspaceFolderContextMenu {
  nodeId: string | null;
  parentNodeId: string | null;
  name: string;
  x: number;
...
interface WorkspaceFolderEditor {
  mode: "create" | "rename";
  nodeId: string | null;
  parentNodeId: string | null;
  value: string;
...
function extractFolderIdFromClipboardText(text: string): string {
  const match = text.trim().match(/(?:^|[^A-Za-z0-9_-])(fld_[A-Za-z0-9_-]+)(?:$|[^A-Za-z0-9_-])/);
  if (!match) {
