...
export function isLikelyWorkspaceFileReference(target: string): boolean {
  ...
  return match ? COMMON_FILE_EXTENSIONS.has(match[1].toLowerCase()) : false;
}

export function shouldResolveWorkspaceFileReference(target: string): boolean {
  const normalized = target
    .trim()
    .replace(/\\/g, "/")
    .replace(/^\.?\/+/, "");
  return normalized !== ".boxteam" && !normalized.startsWith(".boxteam/");
}
