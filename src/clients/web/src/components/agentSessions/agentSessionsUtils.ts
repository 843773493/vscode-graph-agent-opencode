import type { GatewayWorkspace, Session, SessionCatalogNode } from '../../types/backend';
...
export interface SessionCatalogTimePresentation {
  label: string;
  title: string;
}
...
export function sessionCatalogTimePresentation(
  node: SessionCatalogNode,
): SessionCatalogTimePresentation | null {
  const timestamp = node.updated_at ?? node.created_at;
  if (!timestamp) {
    return null;
  }
  const label = formatTime(timestamp);
  const title = formatDateTime(timestamp);
