...
const DEFAULT_BACKEND_PORT = 8015;
const DEFAULT_BACKEND_URL = "http://127.0.0.1:8015";
...
function resolveWorkspaceRoot(args) {
  const raw = args.get("workspace-root") || process.env.BOXTEAM_BROWSER_WORKSPACE_ROOT || process.env.WORKSPACE_ROOT;
  if (typeof raw !== "string" || raw.trim() === "" || raw === "true") {
    throw new Error(
      "browser frontend 启动必须显式提供 workspace root："
...
function resolveAssetRoot(args) {
  ...
  if (typeof raw !== "string" || raw.trim() === "" || raw === "true") {
    throw new Error(
      "browser frontend 启动必须显式提供 asset root："
...
async function main() {
  ...
  const host = args.get("host") || process.env.BOXTEAM_BROWSER_FRONTEND_HOST || DEFAULT_HOST;
  const port = Number(args.get("port") || process.env.BOXTEAM_BROWSER_FRONTEND_PORT || DEFAULT_PORT);
  const backendUrl =
    args.get("backend-url") ||
    process.env.BOXTEAM_BROWSER_BACKEND_URL ||
    DEFAULT_BACKEND_URL;
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      const vendorMap = new Map([
        ["/vendor/codicon/codicon.css", path.join(assetRoot, "src", "clients", "web", "node_modules", "@vscode", "codicons", "dist", "codicon.css")],
        ["/vendor/codicon/codicon.ttf", path.join(assetRoot, "src", "clients", "web", "node_modules", "@vscode", "codicons", "dist", "codicon.ttf")],
      ]);
  ...
  server.listen(port, host, () => {
    console.log(`[browser-frontend] listening on http://${host}:${port}`);
    console.log(`[browser-frontend] backend ${backendUrl}`);
  });
}
