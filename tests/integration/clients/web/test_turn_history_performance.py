...
def test_long_turn_session_keeps_composer_responsive_and_history_incremental(
    ...
    if chromium_path is None:
        pytest.fail(
            "Turn 历史性能集成测试需要 Chromium；设置 PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH"
        )
    ...
    _configure_stub_provider(
        Path(integration_workspace_config_path),
    ...
    with openai_chat_stub(model_port):
        ...
        try:
            ...
            environment.update(
                {
                "BOXTEAM_INTEGRATION_BASE_URL": gateway_url,
                "BOXTEAM_INTEGRATION_METRICS_PATH": str(metrics_path),
                "BOXTEAM_INTEGRATION_SCREENSHOT_PATH": str(screenshot_path),
                "BOXTEAM_INTEGRATION_LOCAL_TOKEN": LOCAL_TOKEN_HEADERS[
                    "X-Local-Token"
                ],
                "BOXTEAM_INTEGRATION_FIXTURE": json.dumps(
                        fixture,
                        ensure_ascii=False,
                    ),
            ...
            try:
                result = subprocess.run(
                    ["node", "tests/integration/clients/web/turn_history_performance.mjs"],
                    cwd=project_root,
                    env=environment,
                    capture_output=True,
                    text=True,
