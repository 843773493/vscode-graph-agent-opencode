...
def test_symlink_path_is_rejected_without_touching_external_files(
    ...
    storage = migration_setup()
    external = storage.root("target").parent / "external"
    external.mkdir()
    sentinel = external / "sentinel"
    sentinel.write_bytes(b"must not change")
    paths = {
        "source_jsonl": storage.jsonl_path("source"),
        "source_index": storage.index_path("source"),
        "source_root": storage.root("source"),
        "target_root": storage.root("target"),
        "audit_root": storage.root("target").parent / "legacy-import",
        "target_lock": storage.root("target").parent / ".rollout.write.lock",
    }
    path = paths[location]
    if path.exists():
...
def test_hardlink_source_is_rejected(migration_setup: Callable) -> None:
    storage = migration_setup()
    os.link(storage.jsonl_path("source"), storage.root("source") / "linked.jsonl")
...
with patch.object(owner, method, exit_during_build if sys.argv[2] == "during_build" else crash):
    _storage(sys.argv[1]).migrate_legacy_to_v2("source", target_thread_id="target")
"""
    process = subprocess.run(
        ["uv", "run", "python", "-c", script, str(storage.sessions_dir), phase],
        capture_output=True,
        text=True,
        timeout=30,
        check=False,
    ...
    target_before = artifact_manifest(storage.root("target")) if installed else None
    recovered = _storage(storage.sessions_dir).recover_legacy_imports("target")
...
def test_post_install_error_cannot_erase_published_history(
    ...
    with pytest.raises(OSError, match="post-rename"):
        storage.migrate_legacy_to_v2("source", target_thread_id="target")
    before = artifact_manifest(storage.root("target"))
    assert migration_audits(storage)[0]["status"] == "installed_audit_failed"
    storage.recover_legacy_imports("target")
    assert artifact_manifest(storage.root("target")) == before
...
def test_source_change_before_install_is_detected(
    ...
    def mutate(*args: object) -> None:
        validate(*args)
        with storage.jsonl_path("source").open("ab") as stream:
...
def test_recovery_rejects_replaced_artifact_symlink(
    ...
    with pytest.raises(OSError, match="post-rename"):
        storage.migrate_legacy_to_v2("source", target_thread_id="target")
    target_file = storage.root("target") / filename
    external = storage.root("target").parent / f"external-{filename}"
...
def test_unmapped_metadata_is_protected_and_explicitly_lossy(
    ...
    storage = migration_setup(records)
    original = artifact_manifest(storage.root("source"))
    result = storage.migrate_legacy_to_v2("source", target_thread_id="target")
    assert result["lossless"] is False
    assert "metadata.private_extension" in json.dumps(result["loss"])
    assert secret not in json.dumps(result)
    assert secret.encode() not in storage.jsonl_path("target").read_bytes()
    assert secret.encode() not in storage.index_path("target").read_bytes()
    assert artifact_manifest(storage.root("source")) == original
...
def test_strict_manifest_envelope_coordinates(
    ...
    if corruption in {"duplicate_key", "unfinished_line"}:
        lines = storage.jsonl_path("source").read_bytes().splitlines(keepends=True)
        lines[0] = (
            lines[0].replace(
        ...
        storage.jsonl_path("source").write_bytes(b"".join(lines))
        with sqlite3.connect(storage.index_path("source")) as connection:
            connection.execute(
...
def test_legacy_hashes_and_source_coordinates_are_frozen(
    ...
    candidate = storage.legacy_migration_report("source")["candidates"][0]
    assert candidate["candidate_key"] == f"legacy-missing-turn:{message_hash}"
    assert candidate["legacy_seed_hash"] == seed
    first = storage.migrate_legacy_to_v2("source", target_thread_id="target")[
        "migrated"
    ][0]
    session_bundle_factory(storage.sessions_dir, "other-target")
    second = storage.migrate_legacy_to_v2("source", target_thread_id="other-target")[
        "migrated"
    ][0]
    ...
    root = storage.read_items("target")[0]
...
def test_raw_quarantine_is_complete_private_and_reported(
    ...
    storage = migration_setup(records)
    original = storage.jsonl_path("source").read_bytes()
    result = storage.migrate_legacy_to_v2("source", target_thread_id="target")
    snapshot = (
        storage.root("target").parent / result["raw_artifact_ref"] / "rollout.jsonl"
    )
    ...
    if kind not in {"request_context", "provider_metadata"}:
        ...
        assert len(storage.read_items("target")) == 1
...
def test_full_copy_lossless_gate_preserves_overlay_raw_and_refuses_install(
    ...
    original = storage.index_path("source").read_bytes()
    ...
    assert not storage.root("target").exists()
    assert storage.index_path("source").read_bytes() == original
