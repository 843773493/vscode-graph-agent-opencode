...
def protected_source(fork_workspace, session_bundle_factory, protected_key):
    ...
    with RolloutCheckpointSaver(sessions, protected_detail_key=protected_key) as saver:
        ...
        plan = saver.compose_committed_context_plan(SOURCE_SESSION_ID, plan_id="source-plan")
        plan = replace(
        ...
        record = detail_record_from_mapping(
            saver._storage.get_context_plan_detail(
                SOURCE_SESSION_ID, detail_ref=entry.detail_ref
            )
        )
...
async def test_full_copy_protected_detached_restarts_without_source(
    ...
    target = saver._storage.root(TARGET_SESSION_ID)
    ...
    with RolloutCheckpointSaver(
        ...
        assert (
            restarted.read_context_plan_detail(
                TARGET_SESSION_ID, detail_ref=target_record.detail_ref, include_sensitive=True
            )["detail"]
            == body
        )
        restored = restarted.get_context_assembly(TARGET_SESSION_ID, assembly_id=assembly_id)
        projected, losses = restarted.project_context_plan_with_diagnostics(
            TARGET_SESSION_ID, restored.as_sealed_plan()
        )
        assert not losses
        ...
        with pytest.raises(PermissionError):
            restarted.read_context_plan_detail(
                TARGET_SESSION_ID, detail_ref=target_record.detail_ref
            )
...
async def test_protected_failure_never_installs_target(protected_source, failure):
    ...
    assert not tuple(saver._storage.root(TARGET_SESSION_ID).parent.glob(".fork-staging-*"))
    assert _artifacts(source) == original
...
async def test_pinned_protected_copy_retains_lineage_not_shared_body(protected_source):
    ...
    with saver._storage._connect(SOURCE_SESSION_ID, "", read_only=True) as connection:
        ...
        assert rows == [(TARGET_SESSION_ID, "active")]
...
async def test_failure_before_install_has_no_publication(
    ...
    original = _artifacts(saver._storage.root(SOURCE_SESSION_ID))
    target = saver._storage.root(TARGET_SESSION_ID)
    ...
    assert _artifacts(saver._storage.root(SOURCE_SESSION_ID)) == original
...
def test_capability_rejects_tampered_source_manifest(protected_source, field, value):
    ...
    with pytest.raises((ValueError, DetailUnavailableError)):
        saver._detail_store.fork_detail_capability().prepare_detail(
            ...
            target_ref=DetailRef(TARGET_SESSION_ID, "target-assembly", "target-detail"),
...
def test_capability_cannot_reuse_source_session_key(protected_source):
    ...
    ).get(SOURCE_SESSION_ID, create=False)
...
def header_detail_factory(protected_source):
    ...
    def create(*, required: bool):
        ...
        record = detail_record_from_mapping(
            saver._storage.get_context_plan_detail(
                SOURCE_SESSION_ID, detail_ref=detail_ref_from_key(key)
            )
        )
        return saver, record
...
async def test_optional_header_preserves_explicit_availability(
    ...
    if not available:
        (
            saver._storage.root(SOURCE_SESSION_ID).parent
    ...
    saver.get_context_assembly(TARGET_SESSION_ID, assembly_id=assembly_id).validate_hashes()
    if not available:
...
async def test_required_header_unavailable_rejects_restore_and_fork(
    ...
    with pytest.raises(RuntimeError, match="detail-unavailable"):
        saver.get_context_assembly(SOURCE_SESSION_ID, assembly_id=record.assembly_id)
    with pytest.raises(RuntimeError, match="detail-unavailable"):
        await saver.afork(
            source_session_id=SOURCE_SESSION_ID,
            target_session_id=TARGET_SESSION_ID,
...
async def test_fork_does_not_copy_unregistered_detail_orphans(protected_source):
    ...
    assert not tuple(saver._storage.root(TARGET_SESSION_ID).rglob("unregistered-detail"))
