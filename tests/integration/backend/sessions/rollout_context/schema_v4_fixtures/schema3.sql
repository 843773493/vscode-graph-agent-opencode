-- 冻结于 schema3 激活期间的真实 sqlite_master；不是 schema4 DDL 改版号。
-- 来源：test_schema_v3_crash_recovery 的已完成 schema2→3 产物（无 draft tables）。
-- 只冻结结构，业务数据由当前真实 domain/storage 生成后装入本历史结构。
CREATE TABLE assembly_item_refs (
...
CREATE TABLE context_assembly_contributions (
            assembly_id TEXT NOT NULL,
...
CREATE TABLE context_assembly_selections (
            assembly_id TEXT NOT NULL,
...
CREATE TABLE context_plan_details (
...
CREATE TABLE context_view_items (
...
CREATE TABLE fork_identity_mappings (
...
CREATE TABLE legacy_migration_reports (
...
CREATE TABLE operation_anchors (
...
CREATE TABLE turn_execution_links (
            turn_id TEXT NOT NULL,
            execution_id TEXT NOT NULL,
...
CREATE INDEX assembly_item_refs_contribution_index ON assembly_item_refs(assembly_id, contribution_id);

CREATE INDEX assembly_item_refs_detail_index ON assembly_item_refs(assembly_id, detail_ref);

CREATE INDEX assembly_item_refs_item_index ON assembly_item_refs(ref_type, ref_id);

CREATE INDEX assembly_item_refs_ordinal_index ON assembly_item_refs(assembly_id, ref_ordinal);

CREATE INDEX checkpoint_channels_view_index ON checkpoint_channels(context_view_id);

CREATE INDEX checkpoint_commit_index ON checkpoints(commit_id DESC);

CREATE INDEX context_assemblies_turn_index ON context_assemblies(turn_id, created_at);

CREATE INDEX context_assembly_contributions_ordinal_index ON context_assembly_contributions(assembly_id, contribution_ordinal);

CREATE INDEX context_assembly_contributions_source_index ON context_assembly_contributions(source_kind, source_revision);

CREATE INDEX context_assembly_selections_contribution_index ON context_assembly_selections(assembly_id, contribution_id);

CREATE UNIQUE INDEX context_assembly_selections_contribution_unique ON context_assembly_selections(assembly_id, contribution_id) WHERE contribution_id IS NOT NULL;

CREATE INDEX context_assembly_selections_ref_index ON context_assembly_selections(ref_type, ref_id);

CREATE UNIQUE INDEX context_contributions_assembly_ordinal_unique
...
CREATE UNIQUE INDEX context_contributions_source_ordinal_unique
            ON context_contributions(source_ordinal)
            WHERE source_ordinal IS NOT NULL;

CREATE INDEX context_view_items_ordinal_index ON context_view_items(view_id, logical_item_ordinal);

CREATE INDEX context_view_turns_ordinal_index ON context_view_turns(view_id, logical_turn_ordinal);

CREATE INDEX context_view_turns_turn_index ON context_view_turns(turn_id, view_id);

CREATE INDEX executions_turn_index ON executions(turn_id, attempt);

CREATE INDEX fork_identity_source_index ON fork_identity_mappings(source_session_id, entity_type, source_local_id);

CREATE INDEX fork_identity_target_index ON fork_identity_mappings(target_session_id, entity_type, target_local_id);

CREATE INDEX fork_materializations_status_index ON fork_materializations(status, created_at);

CREATE INDEX item_catalog_commit_index ON item_catalog(commit_id, item_sequence);

CREATE INDEX item_catalog_group_index ON item_catalog(message_group_id, item_sequence);

CREATE INDEX item_catalog_operation_index ON item_catalog(operation_anchor_capable, item_sequence);

CREATE INDEX item_catalog_semantic_index ON item_catalog(semantic_kind, item_sequence);

CREATE INDEX item_catalog_status_index ON item_catalog(status, item_sequence);

CREATE INDEX item_catalog_turn_index ON item_catalog(turn_id, item_sequence);

CREATE INDEX item_catalog_turn_scope_index ON item_catalog(turn_id, turn_scope, item_sequence);

CREATE INDEX item_parts_hash_index ON item_parts(content_hash);

CREATE INDEX item_projections_semantic_index ON item_projections(semantic_kind, item_sequence);

CREATE INDEX item_projections_status_index ON item_projections(status, item_sequence);

CREATE INDEX item_projections_turn_index ON item_projections(turn_id, item_sequence);

CREATE INDEX messages_offset_index ON messages(jsonl_offset);

CREATE INDEX messages_role_index ON messages(role, message_sequence);

CREATE INDEX messages_turn_index ON messages(turn_id, message_sequence);

CREATE INDEX model_calls_execution_index ON model_calls(execution_id, attempt);

CREATE INDEX pending_writes_checkpoint_index ON pending_writes(checkpoint_id, task_path, write_index);

CREATE INDEX source_overlays_epoch_index ON source_overlays(session_id, source_overlay_epoch);

CREATE INDEX tool_calls_id_index ON tool_calls(tool_call_id, assistant_message_sequence);

CREATE INDEX tool_calls_result_sequence_index ON tool_calls(result_message_sequence);

CREATE INDEX tool_set_snapshots_assembly_index ON tool_set_snapshots(assembly_id, tool_set_snapshot_id);

CREATE INDEX turn_records_ingress_index ON turn_records(accepted_ingress_id);

CREATE INDEX turn_records_ordinal_index ON turn_records(turn_ordinal);

CREATE INDEX turns_ordinal_index ON turns(turn_ordinal);
