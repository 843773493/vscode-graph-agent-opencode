...
from app.agents.agent_tools import create_exec_command_tool, create_write_stdin_tool
...
def terminal_manager_processes(
    ...
    backend_process = subprocess.Popen(
        ...
        cwd=project_root / "src" / "workspace-services" / "terminal" / "server",
        env=env,
...
@pytest.mark.asyncio
async def test_exec_command_tool_is_visible_and_resource_can_be_attached(
    ...
    assert "exec_command" in tool_ids
    assert "write_stdin" in tool_ids
    assert "list_terminal_sessions" in tool_ids
    assert "kill_terminal" in tool_ids
    ...
    result = await tool.ainvoke(
        {
            "cmd": (
                "printf 'BOXTEAM_TERMINAL_OK\\n'; "
                "read value; printf 'BOXTEAM_STDIN:%s\\n' \"$value\"; sleep 20"
            ),
            "yield_time_ms": 1_000,
        }
    )
    ...
    assert "__BOXTEAM_CMD_" not in result["output"]
    terminal_id = result["session_id"]
    assert result["terminal_id"] == terminal_id
    assert result["chunk_id"] != terminal_id

    write_stdin = create_write_stdin_tool(
        session_id=session_id,
        terminal_client=TerminalManagerClient(),
    )
    stdin_result = await write_stdin.ainvoke(
        {
            "session_id": terminal_id,
            "chars": "boxteam-input\r",
            "yield_time_ms": 1_000,
        }
    )
    assert stdin_result["session_id"] == terminal_id
    assert stdin_result["terminal_id"] == terminal_id
    assert stdin_result["chunk_id"] != terminal_id
    assert "BOXTEAM_STDIN:boxteam-input" in stdin_result["output"]
    ...
    frontend_status, html = await asyncio.to_thread(
        _read_http_text,
    ...
    _, snapshot_body = await asyncio.to_thread(
        _read_http_text,
        f"http://127.0.0.1:{backend_port}/api/terminals/{terminal_id}",
    )
    ...
    _, deleted_body = await asyncio.to_thread(
        _read_http_text,
        f"http://127.0.0.1:{backend_port}/api/terminals/{terminal_id}",
    )
    deleted_snapshot = json.loads(deleted_body)["data"]
...
async def test_session_resources_restore_missing_terminal_refs_from_agent_state(
    ...
    missing_snapshot = json.loads(missing_body)["data"]
