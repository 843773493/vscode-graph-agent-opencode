...
from tests.e2e.processes import kill_process_on_port, terminate_process, wait_for_http_ok
...
def dynamic_page_server(request: pytest.FixtureRequest) -> Generator[str, None, None]:
    port = e2e_port_block_for_file(Path(request.node.fspath)).port(62)
...
def test_browser_resource_governance_meets_first_ci_budget(
    ...
    port = e2e_port_block_for_file(Path(request.node.fspath)).port(60)
