...
from tests.unit.services.business.session_turn_history.test_turn_history_service import (
    _build_service,
    _completed,
    _event,
    _LegacyMessages,
...
@pytest.mark.asyncio
async def test_old_projection_version_rebuilds_agent_end_behind_cursor(
    tmp_path: Path,
    session_bundle_factory,
) -> None:
    sessions_dir = tmp_path / "sessions"
    ...
    completed = _completed(1).model_copy(
        update={"timestamp": created.timestamp + timedelta(seconds=2)}
    )
    trace = _TraceEvents([created, agent_end, completed])
    store = TurnHistoryStore(sessions_dir)
    projector = TurnHistoryProjector(store)
    projector.record_event("session_1", created, source_offset=1)
    # 模拟 v1 投影：agent_end 被跳过，但后续 job_completed 已推进权威游标。
    projector.record_event("session_1", completed, source_offset=3)
    store.set_projection_status("session_1", "ready")
    store.mark_history_initialized("session_1", projection_version=1)
    service = _build_service(
        sessions_dir,
        store=store,
        projector=projector,
    ...
    old_snapshot, needs_completion = await service.get_details(
        "session_1",
        ["job_1"],
    )
    assert needs_completion is True
    assert all(item.type != "agent_end" for item in old_snapshot.items[0].items)
    assert store.projection_status("session_1") == "partial"

    await service.complete_migration("session_1")

    rebuilt, needs_completion = await service.get_details("session_1", ["job_1"])
    assert needs_completion is False
    rebuilt_agent_end = next(
        item for item in rebuilt.items[0].items if item.type == "agent_end"
    )
    assert rebuilt_agent_end.raw["payload"]["token_usage"]["total_tokens"] == 7030
    assert store.projection_version("session_1") == CURRENT_TURN_PROJECTION_VERSION


@pytest.mark.asyncio
async def test_newer_projection_version_fails_instead_of_reading_incompatibly(
    tmp_path: Path,
    session_bundle_factory,
) -> None:
    sessions_dir = tmp_path / "sessions"
    ...
    store.mark_history_initialized(
        "session_1",
        projection_version=CURRENT_TURN_PROJECTION_VERSION + 1,
    )
    service = _build_service(
        sessions_dir,
        store=store,
    ...
    with pytest.raises(RuntimeError, match="投影版本高于当前程序支持范围"):
